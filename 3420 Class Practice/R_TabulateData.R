...
# IST 3420-Intro to Data Science & Management, Langtao Chen  #
##############################################################
...
cbind(gear.freq, gear.rel.freq)  # Print frequency and relative frequen

combined_col <- as.data.frame(cbind(gear.freq, gear.rel.freq)) # Create data frame with both columns
...
mpg.cut <- cut(mpg, breaks, right = FALSE) # right = FALSE includes in second subrange instead of first,
# example 15. 15 Goes to 15-20 subrange instead of in 10-15 to keep from their being overlap

# Calculate frequency in each sub-interval
mpg.freq <- table(mpg.cut)
mpg.freq
mpg.freq <- cbind(mpg.freq)  # Print in column format
mpg.freq <- data.frame(mpg.freq)
row.names(mpg.freq) <- c("10-15", "15-20", "20-25", "25-30", "30-35")
