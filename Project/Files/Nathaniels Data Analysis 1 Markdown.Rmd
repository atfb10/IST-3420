---
title: "IMDb Exploratory Analysis - Milestone M2"
...
Load in Packages:
```{r}
# Note: User may need install the required packages
library(ggplot2)
library(plyr)
library(dplyr)
library(plotly) 
...
```{r}
movie_df <- read.csv(file = "movie_metadataCSV.csv", header = TRUE)
```

Show the initial structure of the data frame:
```{r}
str(movie_df)
```
...
```{r}
# Show the original total number of titles, including duplicates
length(movie_df$movie_title)

# Show number of unique titles
length(unique(movie_df$movie_title))

# Show number of duplicated titles, some of which may be included 2 or more times
anyDuplicated(movie_df$movie_title)
```

After verifying redundant values exist, We need to remove duplicates to avoid skew in our analysis.
```{r}
# This statement removes movie titles that are duplicates
movie_df <- movie_df[!duplicated(movie_df$movie_title),]
```

Show the summary statistics now that the dataset has been cleansed.
```{r}
summary(movie_df)
```
...
```{r}
# Create smaller data frame so there is not an overwheliming amount of values 
small_movie_df <- head(movie_df, n = 500)
...
ggplot(small_movie_df, aes(x = movie_facebook_likes, y = num_critic_for_reviews)) +
  ...
  ggtitle("Facebook Likes and # of Critc Reviews Regression") +
  ylab("# of Critic Reviews") +
  xlab("Movie Likes on Facebook")
```
...
Find out which actors have the highest average movie score
```{r}
...
Find out which directors have the highest average movie score
```{r}
# Find actors with highest average movie score 
...
Find out the average Imdb score based on content rating
```{r}
...
Show spread of scores to detect if outliers could be effecting the content score averages
```{r}
